...
def main(ARGS):
    ...
    for frame in frames:
        if frame is not None:
            ...
            #def tts_reply():
                #tts = gTTS(text=VARIABLE_DICKS, lang='en')
            ...
            def youtubedl(target_url):
                ...
                tts = gTTS(text='Downloading '+target_url, lang='en')
                tts.save("ttstemp.wav")
                os.system("mpg321 -q ttstemp.wav")
                os.system("rm ttstemp.wav")
                os.system("/usr/local/bin/youtube-dl -q -o \"/home/pi/Music/%(title)s\" $(googler -w youtube.com --json "+target_url+" | jq -r '.[].url' | head -1)")
                #finished download
                logging.info("Download complete.")
                tts = gTTS(text='Download complete.', lang='en')
                tts.save("ttstemp.wav")
                os.system("mpg321 -q ttstemp.wav")
                os.system("rm ttstemp.wav")
                #converting
                logging.info("Converting song...")
                tts = gTTS(text='Converting '+target_url+' to MP3 format', lang='en')
                tts.save("ttstemp.wav")
                os.system("mpg321 -q ttstemp.wav")
                os.system("rm ttstemp.wav")
                os.system("find /home/pi/Music/ -type f -name \"* *\" | while read file; do mv \"$file\" ${file// /_}; done")
                #subprocess.run(['find /home/pi/Music/ -type f -name "* *" | while read file; do mv "$file" ${file// /_}; done'])
                subprocess.run(['sh', '/home/pi/Music/mp3con.sh'])
                #cmd = "/home/pi/Music/replace_spaces.sh"
                #os.system(cmd)
                #cmd2 = "/home/pi/Music/mp3con.sh"
                #os.system(cmd2)
                #conversion should begin here, in fucking THEORY
                #os.system("find /home/pi/Music/ -type f -name "* *" | while read file; do mv "$file" ${file// /_}; done")
                tts = gTTS(text='Conversion complete and file name modified to read properly.', lang='en')
                tts.save("ttstemp.wav")
                os.system("mpg321 -q ttstemp.wav")
                os.system("rm ttstemp.wav")
            ...
            if text.find("update dashboard") != -1:
                x = threading.Thread(target=funcdashboard)
                x.start()
